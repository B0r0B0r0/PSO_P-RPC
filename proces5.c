...
void *ReturnRequest(void * arg)
{
        ...
        char* key;
        if(strcmp(params->operation,"encrypt")==0)
                key=encrypt(str);
        else if(strcmp(params->operation,"decrypt")==0)
                key=decrypt(str);

        int n=write(fd,key,strlen(key)+1);
        if(n<0)
        {
        ...
        while (1)
    {
        ...
        if(n>0)
        {
           ...
           // c=strtok(aux,"/");//acum avem protocolul
            ...
            if(strcmp("encrypt",numefunc)==0 || strcmp("decrypt",numefunc)==0);//daca functia ceruta este a noastra atunci o rulam
            {
               ...
               params.fd=myfd;
               params.str=strdup(c);
               params.operation=strdup(numefunc);
...
int main(int argc, char** argv)
{
    ...
    sprintf(fd_p,"%d",fd);
    strcpy(buffer,fd_p);
    strcat(buffer,"/2");//asta este numarul de functii per proces
    //strcat(buffer,fd_p);
    strcat(buffer,"/encrypt/decrypt");//functia si parametrii
